		...
		if (!$is_insert)
		{
			...
			foreach ($fms as $fm)
			{
				if ((!$fm->IsPrimaryKey) && $fm->FieldType != FM_CALCULATION)
				{
					...
					try
					{
						// alcalbg handle null values
						if ($val === null)
							$sql .= $delim . "`" . $fm->ColumnName . "` = null";
						else
			...
			foreach ($fms as $fm)
			{
				...
				if ( (!$fm->IsPrimaryKey) || $force_insert)
				{
					...
					if ($fm->FieldType != FM_CALCULATION)
					{
						...
						// alcalbg
						// these fields are auto-populated so we don't need them unless
						// a specific value was provided
						// if ($fm->IsAutoInsert && $fm->DefaultValue === $val) continue;
