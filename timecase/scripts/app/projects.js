...
var page = {
	...
	init: function()
	{
		...
		$('#projectDetailDialog').on('show',function(){
			page.dialogIsOpen = true;
		});

		// when the model dialog is closed, let page know and reset the model view
		$('#projectDetailDialog').on('hidden',function(){
			$('#modelAlert').html('');
		...
		this.collectionView.on('rendered',function(){
			...
			$('table.collection tbody tr').click(function(e) {
				e.preventDefault();
				var m = page.projects.get(this.id);
	...
	showDetailDialog: function(m) {
		...
		$('#projectDetailDialog').modal({ show: true });

		// if a model was specified then that means a user is editing an existing record
		// if not, then the user is creating a new record
		page.project = m ? m : new model.ProjectModel();

		page.modelView.model = page.project;

		if (page.project.id == null || page.project.id == '')
		{
	...
	renderModelView: function(isExistingRecord)
	{
		...
		// populate the dropdown options for statusId
		// TODO: load only the selected value, then fetch all options when the drop-down is clicked
		var statusIdValues = new model.StatusCollection();
		statusIdValues.fetch({
			success: function(c){
				var dd = $('#statusId');
				dd.append('<option value=""></option>');
				c.forEach(function(item,index)
		...
		if (isExistingRecord)
		{
			...
			$('#deleteProjectButton').click(function(e) {
				e.preventDefault();
				$('#confirmDeleteProjectContainer').show('fast');
			});

			$('#cancelDeleteProjectButton').click(function(e) {
				e.preventDefault();
				$('#confirmDeleteProjectContainer').hide('fast');
			});
		...
		else
		{
			...
			$('#deleteProjectButtonContainer').hide();
		}
	},
	...
	updateModel: function()
	{
		...
		page.project.save({
			...
			success: function(){
				...
				if (isNew) { page.projects.add(page.project) }
